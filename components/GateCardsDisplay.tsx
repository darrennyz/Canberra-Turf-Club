...
export default function GateCardsDisplay({ gateCards, gateRevealed, gateThresholds, highlightIndex }: Props) {
  return (
    <div className="flex flex-col gap-2">
      <div className="text-xs text-green-700 text-center mb-1 pixel-text">⚠ GATE CARDS ⚠</div>
      <div className="flex gap-2 justify-center flex-wrap">
        {[0, 1, 2, 3, 4].map(i => (
          <div
            key={i}
            className={`flex flex-col items-center gap-1 ${highlightIndex === i ? 'ring-2 ring-yellow-500 rounded' : ''}`}
          >
            <CardDisplay
